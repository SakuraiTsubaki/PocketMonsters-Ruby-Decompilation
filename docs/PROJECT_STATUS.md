...
The original Japanese release is the historical comparison baseline. All confirmed regional, language, and revision branches are tracked separately rather than being silently merged into one generic Ruby target.

## Version inventory

| Target | Region | Language | Revision / update | Verification | Notes |
| --- | --- | --- | --- | --- | --- |
| Pokémon Ruby | Japan | Japanese | Rev 0 / v1.0 | Observed | Primary historical baseline; `AGB-AXVJ-0`; SHA-1 recorded in `VERSIONS.md` |
| Pokémon Ruby | Japan | Japanese | Rev 1 / v1.1 | Observed | Japanese revision branch; `AGB-AXVJ-1`; SHA-1 recorded in `VERSIONS.md` |
| Pokémon Ruby | North America | English | Rev 0 | Reproduced upstream / local verification pending | `pret/pokeruby` matching target; public upstream SHA-1 known |
| Pokémon Ruby | Other official regional/language branches | English/French/German/Italian/Spanish | Multiple | Observed / inventory in progress | Product-code/revision matrix seeded in `VERSIONS.md`; unresolved mappings remain explicit |

## Progress

- [x] Establish Japanese historical baseline identity for Ruby Rev 0 and Rev 1
...
- [ ] Complete authoritative version/revision inventory for every official Ruby branch
...
- [`versions/JAPANESE_BASELINE.md`](versions/JAPANESE_BASELINE.md) — Japanese Ruby baseline identity, provenance, public-source coverage, and open questions
...
1. Finish Ruby's complete region/language/revision identity matrix, including unresolved English `AXVE`/`AXVP` market-to-binary mappings.
2. Cross-check hashes and product identifiers from multiple independent public sources.
3. Document the Japanese Rev 0 ↔ Rev 1 delta and the Berry/RTC fix at source/function level.
4. Inventory public Japanese Ruby source/symbol/data research by subsystem.
5. Begin executable/data mapping only after the relevant target identities are stable enough to prevent cross-version contamination.
