...
- Exact release, region, revision, and build: **Japanese retail cartridge, `AXVJ`, revision 0, identified**

The selected reconstruction target is the locally observed Japanese retail cartridge (`AXVJ`, revision 0). Its hashes and header identity are recorded in `config/target.json`; the status remains `identified`, not independently `verified`.
